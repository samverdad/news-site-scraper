...
class NewsSiteScraperSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for item_url in response.css(selector).getall():
            yield scrapy.Request(url=get_proxy_url('https://www.freemalaysiatoday.com' + item_url), callback=self.scrape, cb_kwargs={'url': item_url})
    ...
    def scrape(self, response, url):
        yield {
            ...
            'url': 'https://www.freemalaysiatoday.com' + url,
        }
